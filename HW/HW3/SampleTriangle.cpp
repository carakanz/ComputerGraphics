...
SampleTriangle::SampleTriangle()
    : AppBase("SimpleTriangle", 500, 400, 0, true)
{

}

void SampleTriangle::Initialize()
{
    CreateModel();
    CreateTexture();
    CreateSampler();
...
void SampleTriangle::OnKeyUp(int key)
{
...
void SampleTriangle::OnResize(int width, int height)
{
...
void SampleTriangle::Update(float timeElapsed)
{
    ...
    UniformBuffer ub = {};
    ub.model = glm::rotate(glm::mat4(1.0f), elapsedTime * glm::radians(0.05f), glm::vec3(0.0f, 1.0f, 0.0f));
    ub.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void SampleTriangle::CreateModel()
{
    if (!m_model.LoadFromFile("Models/apple.obj", AppBase::GetDevice()))
        FATAL("Model load from file failed");
}

void SampleTriangle::CreateTexture()
{
    // Load image from disk.
    int width, height, channels;
    auto pixelData = stbi_load("Textures/texture.jpg", &width, &height, &channels, 4);
...
void SampleTriangle::CreateSampler()
{
...
void SampleTriangle::CreateUniformBuffer()
{
...
void SampleTriangle::CreatePipeline()
{
    // Create the graphics pipeline.
    if (!AppBase::CreatePipeline(
        { { "Shaders/SampleTriangle.vert", VK_SHADER_STAGE_VERTEX_BIT },
        { "Shaders/SampleTriangle.frag", VK_SHADER_STAGE_FRAGMENT_BIT } },
...
void SampleTriangle::CreateCommandBuffer()
{
